document.onload = (function(d3, saveAs, Blob, undefined) {
	...
	$.getJSON("nodes_kmeans_20.json", function(result) {
		$.each(result, function(i, node) {
			node = nodes[parseInt(node.clusterid)]
					|| (nodes[parseInt(node.clusterid)] = {
						title : parseInt(node.clusterid).toString(),
						id : parseInt(node.clusterid),
						size : parseInt(0),
						x : toScreenX(node.x),
						y : toScreenY(node.y)
					});
		...
		$.getJSON("linkswithin_kmeans_20.json", function(result) {
			$.each(result, function(i, link) {
				if (link.source == link.target) {
					nodes[parseInt(link.source)].size += parseInt(link.Total);
				} else {
					var edge = {
						source : nodes[parseInt(link.source)],
						target : nodes[parseInt(link.target)],
						total : parseInt(link.Total)
					};
					edges.push(edge);
