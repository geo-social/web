document.onload = (function(d3, saveAs, Blob, undefined) {
	...
	var GraphCreator = function(svg, nodes, edges, mvdata) {
		var thisGraph = this;
		thisGraph.idct = 0;
		thisGraph.flowBreaks = [];
		thisGraph.colorClass = [ "first", "second", "third", "fourth", "fifth" ];

		thisGraph.nodes = nodes || [];
		thisGraph.edges = edges || [];
		thisGraph.mvdata = mvdata || {};
		...
		var dropShadowFilter = defs.append('svg:filter').attr('id',
				'dropShadow');
		dropShadowFilter.append('svg:feGaussianBlur').attr('in', 'SourceAlpha')
				.attr('stdDeviation', 1);
		dropShadowFilter.append('svg:feOffset').attr('dx', 0).attr('dy', 1)
		...
		var parcoords = d3.parcoords()("#pcp").data(pcpdata).render()
				.brushable().on("brush", function(items) {
	...
	GraphCreator.prototype.setFlowBreaks = function(breaks) {
		this.flowBreaks = breaks;
	};
	...
	GraphCreator.prototype.changeTextOfNode = function(d3node, d) {
		...
		var d3txt = thisGraph.svg.selectAll("foreignObject").data([ d ])
				.enter().append("foreignObject").attr("x",
						nodeBCR.left + placePad).attr("y",
						nodeBCR.top + placePad).attr("height", 2 * useHW).attr(
						"width", useHW).append("xhtml:p").attr("id",
						consts.activeEditId).attr("contentEditable", "true")
				.text(d.title).on("mousedown", function(d) {
					d3.event.stopPropagation();
				}).on(
						"keydown",
						function(d) {
	...
	GraphCreator.prototype.svgKeyDown = function() {
		...
		case consts.DELETE_KEY:
			...
			if (selectedNode) {
				thisGraph.nodes
						.splice(thisGraph.nodes.indexOf(selectedNode), 1);
				thisGraph.spliceLinksForNode(selectedNode);
				state.selectedNode = null;
				thisGraph.updateGraph();
			} else if (selectedEdge) {
				thisGraph.edges
						.splice(thisGraph.edges.indexOf(selectedEdge), 1);
	...
	GraphCreator.prototype.updateGraph = function() {
		...
		paths
				...
				.style("opacity", consts.pathOpacity)
				.attr(
						"class",
						function(d) {
							for ( var b = 1; b < thisGraph.flowBreaks.length; b++)
	...
	$.getJSON("data/nodes_kmeans_unique_20.json", function(result) {
		$.each(result, function(i, node) {
			node = nodes[parseInt(node.id)] || (nodes[parseInt(node.id)] = {
				id : parseInt(node.id),
				title : parseInt(node.id).toString(),
				x : toScreenX(node.x),
				y : toScreenY(node.y)
			});
		});

		$.getJSON("data/1929_10_kmeans_20.json", function(result) {
			$.each(result, function(i, link) {
				if (link.source == link.target) {
					for ( var col in link) {
						if (link.hasOwnProperty(col)) {
							if (col != 'source' && col != 'target') {
								nodes[parseInt(link.source)][col] = link[col];
							}
						}
					}
					// nodes[link.source].size += parseInt(link.Total);

					// this part is for pcp test
					// var mymvdata = {
					// name : link.source
					// }
					// for ( var col in link) {
					// if (link.hasOwnProperty(col)) {
					// mymvdata[col] = link[col];
					// }
					// }
					// mvdata[link.source] = mymvdata;
			...
			$.each(result, function(i, link) {
				if (link.source != link.target) {
					...
					edges.push(edge);
					edgeValues.push(parseInt(link.Total));
					cnt++;
				}
			});
			edges.sort(compare);

			var edgebreaks = jenks(edgeValues, 5);
			// var options = '';
			// for ( var x in types) {
			...
			var svg = d3.select("#flowmap").append("svg").attr("width", width)
					.attr("height", height);
			var graph = new GraphCreator(svg, nodes, edges, mvdata);
			graph.setIdCt(nodes.length);
			graph.setFlowBreaks(edgebreaks);
			graph.updateGraph();
